...
function doneTyping() {
    ...
    var url = "/api/product/search-products/" + term;
    var xhr = new XMLHttpRequest();
    if (url == "/api/product/search-products/") {
        return;
    } else {
        xhr.open("GET", url);
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                ...
                data.map((data) => {
                    UlDropdown.append(
                        ul_func(data.product_name, data.variant_id)
                    );
                });
            }
        };
...
function search() {
    const searchTerm = document.querySelector(".searchTerm");
    var url =
        "http://localhost:90/api/product/search-products/" + searchTerm.value;
    var xhr = new XMLHttpRequest();
    xhr.open("GET", url);
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
